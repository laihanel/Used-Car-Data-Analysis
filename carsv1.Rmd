...
```{r}
df %>%  ggplot(aes(year_produced)) + geom_bar(fill = "#00AFBB") + ggtitle('Number of cars available per year') + theme(plot.title = element_text(hjust = 0.5))
```
```{r}

df %>% ggplot(aes(engine_type, price)) + geom_boxplot()
```

```{r}
df %>% ggplot(aes(has_warranty, price)) + geom_boxplot()
```


```{r}
df %>% ggplot(aes(state, price)) + geom_boxplot()
```



#Does Number of photot really make a change on the price
```{r}
df %>% ggplot(aes(number_of_photos, price)) + geom_smooth()
```
```{r}
df %>% ggplot(aes(color,price))+geom_boxplot()

df %>% group_by(color) %>% summarise(price_colorMean=mean(price)) %>% ggplot(aes(x=reorder(color,-price_colorMean),y=price_colorMean)) + geom_col(fill = "#00AFBB")


```
```{r}
df %>% ggplot(aes(transmission,price))+geom_boxplot()
```
```{r}

df %>% group_by(body_type) %>% summarise(body_price_mean = mean(price))%>% ggplot(aes(x = reorder(body_type, -body_price_mean),body_price_mean))+geom_col()
```

```{r}
df %>% ggplot(aes(x=reorder(body_type,-engine_capacity),y=engine_capacity))+geom_boxplot()
```
```{r}
