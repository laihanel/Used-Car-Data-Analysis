...
```
Because many of the expected values are so small, the approximation of p may not be right.



## ANOVA test
```{r}
(df.aov = aov(price ~ manufacturer_name * color, df))
summary(df.aov)

```
Because the p < 0.001, we reject that all colors sell the same price, all manufacturer sell the same price and manufacturers and colors combined sell the same price.
