...
library(olsrr)
ols_plot_resid_stud_fit(model, threshold = NULL, print_plot = TRUE) # replace model
```

# x outlier check
```{r cook's distance}
plot(cooks.distance(model)) # replace model
```
# heteroscedasticity check
```{r residual plot}
resid = residuals(model) # replace model
yfitted = fitted.values(model) # replace model
plot(yfitted, resid, xbar = "fitted value", ybar = "residual")
```
...
library(pls)
pls1 = plsr(price ~ . data = df, validation = "CV")
