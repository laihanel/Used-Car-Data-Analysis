...
The results validated the intuitions behind these methods.The adjusted R-squared `r summary(fit2)$adj.r.squared` and residual standard error `r summary(fit2)$sigma` both decreased, with a still significant linear relationship and significant regression coefficients at large. More importantly, the residual plot displayed an evident improvement. 
...
```{r model-comparison}
library(kableExtra)
d = data.frame(rbind( c(summary(fit1)$adj.r.squared, summary(fit1)$sigma), c(summary(fit2)$adj.r.squared, summary(fit2)$sigma), c(summary(fit3)$adj.r.squared, summary(fit3)$sigma), c(adjrsq_ridge_cv, rse0), c(adjr2, rsePls2)), row.names = c('Base Model','Polynomial Terms ','Interaction Terms','Ridge Regression', 'PLS Regression'))
colnames(d) = c('Adjusted R-Square', 'Residual Standard Error')
kbl(d) %>%
kable_styling(bootstrap_options = c("striped", "hover"))
```

`ridge regression` and `PLS` both have significantly lower adjusted r-squared score which is understandable, since they both sacrifice biases for better confidence, with `PLS` at `r adjr2` edged over `ridge regression` at `r adjrsq_ridge_cv`. However residual standard errors differ a lot. `PLS` did not sacrifice at all at `r rsePls2` while `ridge regression` increased to `r rse0`.
