...
```
There are `length(outliers)` outliers for price in this data set. These data will not be eliminated since they also reflect the actual situation in the used car market. They represent the group that was a relatively new car with a higher price.
...
•	From the Data set we mainly focus on these features as mentioned below to perform Exploratory Data Analysis:
...
PatternSix's future work for this topic is building up a model to predict the price based on the analysis we explored to provide more effective decision-making services for future vehicle buyers and sellers.
