...
library(olsrr)
library(pls)
knitr::opts_chunk$set(warning = F, message = F, comment = NA, tidy = TRUE)
...
## Data Transformation
We can observe that the target variable is not normally distributed which is an assumption of the linear regression model
...
pls1 = plsr(price_normal ~ odometer_value*poly(year_produced, 2)+sin(engine_capacity)+poly(number_of_photos,                                 2)+up_counter+manufacturer_name+color+transmission+engine_fuel+body_type+has_warranty+state+drivetrain+is_exchangeabl            e+state, data = df_normal, validation = "CV")
