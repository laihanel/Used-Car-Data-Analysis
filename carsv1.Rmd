...
Table above give us the basic statistic measures of numeric data. 
...
```{r}
df %>% group_by(engine_capacity) %>% summarize(mean_price_per_capicity = mean(price, na.rm=TRUE)) %>% ggplot(aes(x=engine_capacity,y=mean_price_per_capicity)) +  geom_point(color = "#00AFBB") + labs(title='Avg Price of Car for engine capicity', x='Engine Capicity', y='Mean Price') + theme(plot.title = element_text(hjust = 0.5))
```
...
### Top 10 Body Types by Mean Price
```{r}
...
From the initial analysis, for the above 3 plots there seems to be difference among the true means of thier respective groups. Lets run an anova test on each of the case to check the hypothesis
...
### interaction of manufacturer and body type vs price

```{r echo=TRUE}
df_aov_3 = aov(price ~   manufacturer_name * body_type , df)
summary(df_aov_3)
```
...
Let's try to see if there is any interaction between the manufacturer_name and the body_type with price
```{r}
