...
## Part1 - Conclusion and Discussions
...
tibble(val=price_normal) %>% ggplot(aes(val)) + geom_density()
...
For this curved regression surface, PatternSix used GAM plot solve the transformation required. 
...
The results validated intuitions behind these methods.The adjusted R-squared `r summary(fit2)$adj.r.squared` and residual standard error `r summary(fit2)$sigma` both decreased, with a still significant linear relationship and significant regression coefficients at large. More importantly, the residual plot displayed an evident improvement. 
...
#### RIDGE Regression
The first variation is called ridge regression, which adds penalties to squared value of regression coefficients.
...
#### LASSO Regression
The second variation is called Lasso regression, which adds penalty terms to absolute value of regression coefficients. It is a sort of shrinkage-based linear regression. Data values shrunk towards a central point, such as the mean, in shrinkage. Simple, sparse models are encouraged by the lasso approach (i.e. models with fewer parameters).The purpose of lasso regression is to find the subset of predictors that produces the least amount of prediction error for a quantitative response variable.
...
pcr.RMSEP = RMSEP(pcr1, estimate = "CV")
plot(pcr.RMSEP, main="RMSEP PCR Price", xlab="components")
min_comp = which.min(pcr.RMSEP$val)
min_val = min(pcr.RMSEP$val)
points(min_comp, min(pcr.RMSEP$val), pch=1, col="red", cex=1.5)
```
...
min_val = min(pls.RMSEP$val)
points(min_comp, min(pls.RMSEP$val), pch=1, col="red", cex=1.5)
```

It can be found that `r min_comp` components out of the original 97 independent variables would yield the smallest RMSEP `r min_val`. 
...
rmsePls2 = RMSEP(pls2, ncomp = min_comp)$val[4]
jack.test(pls2)
```

Compared with final model before solving multi-collinearity, the adjusted R-squared is almost identical at `r adjr2` and residual standard error at `r rmsePls2`. Majority of the regression coefficients are still significant.

## Part 2 - Conclusion and discussion
The table below gives the comparison of different models.
```{r model-comparison}
library(kableExtra)
d = data.frame(rbind( c(summary(fit1)$adj.r.squared, summary(fit1)$sigma), c(summary(fit2)$adj.r.squared, summary(fit2)$sigma), c(summary(fit3)$adj.r.squared, summary(fit3)$sigma), c(adjrsq_ridge_cv, rmse0), c(adjr2, rmsePls2)), row.names = c('Base Model','Polynomial Terms ','Interaction Terms','Ridge Regression', 'PLS Regression'))
colnames(d) = c('Adjusted R-Square', 'Residual Standard Error')
kbl(d) %>%
kable_styling(bootstrap_options = c("striped", "hover"))
```

`ridge regression` and `PLS` both have significantly lower adjusted r-squared score which is understandable, since they both sacrifice biases for better confidence. Even though ridge regression has significant lower residual standard error at `r rmse0` than `PLS` at `r rmsePls2`, `PLS` showed better adjusted r-squared value.
...
colnames(d2) = c("OLS, Ridge Regression", 'PCR, PLS')
kbl(d2) %>%
kable_styling(bootstrap_options = c("striped", "hover"))
...
Combined with testing results from different methods shown in the first table, PatternSix decided to choose the `PLS` model with variables of coefficient with at least 99% confidence as the final one. 
